'use client';

import { useEffect, useId, useRef, useState } from 'react';
import { Check, ChevronsUpDown, Loader2, Search, User, X } from 'lucide-react';
import { api } from '@/lib/api';
import { cn } from '@/lib/utils';
import type { EmployeeOption } from '../_lib/types';

interface EmployeeSelectSearchProps {
  value: string;
  onChange: (employeeId: string, employee?: EmployeeOption) => void;
  placeholder?: string;
  emptyLabel?: string;
  disabled?: boolean;
  className?: string;
  initialEmployee?: EmployeeOption | null;
}

export function EmployeeSelectSearch({
  value,
  onChange,
  placeholder = 'Cari nama atau NRK karyawan...',
  emptyLabel = 'Pilih karyawan approver',
  disabled = false,
  className,
  initialEmployee,
}: EmployeeSelectSearchProps) {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState('');
  const [loading, setLoading] = useState(false);
  const [employees, setEmployees] = useState<EmployeeOption[]>([]);
  const [selectedEmployee, setSelectedEmployee] = useState<EmployeeOption | null>(initialEmployee || null);

  const containerRef = useRef<HTMLDivElement>(null);
  const searchInputRef = useRef<HTMLInputElement>(null);
  const dropdownId = useId();

  // Keep selected employee synced if value matches initialEmployee or employees list
  useEffect(() => {
    if (!value) {
      setSelectedEmployee(null);
      return;
    }
    if (initialEmployee && initialEmployee.id === value) {
      setSelectedEmployee(initialEmployee);
      return;
    }
    const found = employees.find(e => e.id === value);
    if (found) {
      setSelectedEmployee(found);
    }
  }, [value, initialEmployee, employees]);

  // Click outside listener
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
        setOpen(false);
      }
    };
    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, []);

  // Fetch employees when open or query changes
  useEffect(() => {
    if (!open) {
      setQuery('');
      return;
    }

    setTimeout(() => searchInputRef.current?.focus(), 50);

    setLoading(true);
    const timer = setTimeout(() => {
      const params = new URLSearchParams({ limit: '50' });
      if (query.trim()) params.set('search', query.trim());

      api
        .get<EmployeeOption[]>(`/employees?${params}`)
        .then(response => {
          const list = response.data || [];
          // Ensure currently selected employee stays in option list if present
          if (selectedEmployee && !list.some(e => e.id === selectedEmployee.id)) {
            setEmployees([selectedEmployee, ...list]);
          } else {
            setEmployees(list);
          }
        })
        .catch(() => setEmployees(selectedEmployee ? [selectedEmployee] : []))
        .finally(() => setLoading(false));
    }, 250);

    return () => clearTimeout(timer);
  }, [open, query, selectedEmployee]);

  const handleSelect = (emp: EmployeeOption | null) => {
    if (emp) {
      setSelectedEmployee(emp);
      onChange(emp.id, emp);
    } else {
      setSelectedEmployee(null);
      onChange('');
    }
    setOpen(false);
  };

  return (
    <div ref={containerRef} className={cn('relative w-full', className)}>
      {/* Dropdown Field Trigger */}
      <div
        role="button"
        tabIndex={disabled ? -1 : 0}
        aria-haspopup="listbox"
        aria-expanded={open}
        aria-controls={dropdownId}
        onClick={() => !disabled && setOpen(!open)}
        onKeyDown={event => {
          if (!disabled && (event.key === 'Enter' || event.key === ' ')) {
            event.preventDefault();
            setOpen(!open);
          }
        }}
        className={cn(
          'flex min-h-10 w-full items-center justify-between gap-2 rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs font-semibold text-slate-800 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/20 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-100 dark:focus-visible:ring-amber-500/30',
          disabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer hover:border-slate-300 dark:hover:border-slate-700',
        )}
      >
        <span className="truncate">
          {selectedEmployee ? (
            <span className="flex items-center gap-1.5">
              <User className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400 shrink-0" />
              <span className="font-bold">{selectedEmployee.nama}</span>
              <span className="text-[10px] text-slate-400">({selectedEmployee.nrk})</span>
            </span>
          ) : (
            <span className="text-slate-400 dark:text-slate-500">{emptyLabel}</span>
          )}
        </span>

        <div className="flex shrink-0 items-center gap-1">
          {selectedEmployee && !disabled && (
            <button
              type="button"
              onClick={event => {
                event.stopPropagation();
                handleSelect(null);
              }}
              className="rounded p-0.5 text-slate-400 hover:bg-slate-200 hover:text-slate-600 dark:hover:bg-slate-800 dark:hover:text-slate-200"
              title="Hapus pilihan"
            >
              <X className="h-3.5 w-3.5" />
            </button>
          )}
          <ChevronsUpDown className="h-3.5 w-3.5 text-slate-400" />
        </div>
      </div>

      {/* Popover Dropdown */}
      {open && (
        <div
          id={dropdownId}
          role="listbox"
          className="absolute z-50 mt-1 max-h-64 w-full overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-900"
        >
          {/* Integrated Search Bar inside Popover */}
          <div className="border-b border-slate-100 p-2 dark:border-slate-800">
            <div className="relative">
              <Search className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
              <input
                ref={searchInputRef}
                type="text"
                value={query}
                onChange={event => setQuery(event.target.value)}
                placeholder={placeholder}
                className="w-full rounded-md border border-slate-200 bg-slate-50 py-1.5 pl-8 pr-3 text-xs text-slate-800 outline-none focus:border-amber-500 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100"
              />
            </div>
          </div>

          {/* List Options */}
          <div className="max-h-48 overflow-y-auto p-1">
            {loading ? (
              <div className="flex items-center justify-center gap-2 py-4 text-xs font-semibold text-slate-400">
                <Loader2 className="h-4 w-4 animate-spin text-amber-500" />
                <span>Mencari karyawan...</span>
              </div>
            ) : employees.length === 0 ? (
              <div className="px-3 py-4 text-center text-xs text-slate-400">Karyawan tidak ditemukan</div>
            ) : (
              employees.map(emp => {
                const isSelected = emp.id === value;
                return (
                  <button
                    key={emp.id}
                    type="button"
                    role="option"
                    aria-selected={isSelected}
                    onClick={() => handleSelect(emp)}
                    className={cn(
                      'flex w-full items-center justify-between rounded-md px-2.5 py-2 text-left text-xs transition-colors',
                      isSelected
                        ? 'bg-amber-50 font-bold text-amber-700 dark:bg-amber-950/40 dark:text-amber-300'
                        : 'text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800/60',
                    )}
                  >
                    <div className="min-w-0 flex-1">
                      <span className="block truncate font-semibold">{emp.nama}</span>
                      <span className="block text-[10px] text-slate-400">
                        NRK: {emp.nrk} {emp.jabatan ? `· ${emp.jabatan}` : ''}
                      </span>
                    </div>
                    {isSelected && <Check className="h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-400" />}
                  </button>
                );
              })
            )}
          </div>
        </div>
      )}
    </div>
  );
}
