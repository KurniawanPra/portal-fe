  ...
  useEffect(() => {
    ...
    async function launch() {
      try {
        ...
        const app = appId
          ? (appsRes.data || []).find(item => item.id === appId)
          : (appsRes.data || []).find(item => item.nama.toLowerCase() === appName.toLowerCase());
        ...
        window.location.replace(`${baseUrl}${separator}token=${encodeURIComponent(tokenRes.data.token)}&appId=${encodeURIComponent(appId)}`);
      } catch (err) {
        if (!cancelled) {
