  ...
  const fetchUsers = useCallback(async () => {
    try {
      const [usersRes, employeesRes] = await Promise.all([
        api.get<ApiUser[]>('/users?limit=500'),
        api.get<EmployeeBrief[]>('/employees?limit=500'),
      ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
        <div className="pointer-events-none absolute inset-x-0 top-0 z-20 h-px bg-gradient-to-r from-transparent via-amber-500/30 to-transparent" />
        ...
        {/* Table */}
        <CrudTable<UserData>
          headers={['No', 'User', 'Role', 'Status', 'MFA / Keamanan', 'Login Terakhir', 'Aksi']}
          loading={loading}
