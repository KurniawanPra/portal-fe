...
interface ApiKategoriAplikasi {
  id: string;
  kode: string;
  label: string;
}

export default function TabKategoriAplikasi() {
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [data, setData] = useState<KategoriAplikasi[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [search, setSearch] = useState('');
  const [modalOpen, setModalOpen] = useState(false);
  const [editTarget, setEditTarget] = useState<KategoriAplikasi | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<KategoriAplikasi | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await api.get<ApiKategoriAplikasi[]>('/master/kategori-aplikasi');
  ...
  const openEdit = (r: KategoriAplikasi) => {
    setEditTarget(r);
    setForm({ nama: r.nama });
    setErrors({});
    setModalOpen(true);
  ...
  const handleSave = async () => {
    ...
    if (!form.nama.trim()) { newErrors.nama = 'Nama Kategori wajib diisi.'; }
    ...
    try {
      ...
      if (editTarget) {
        await api.put(`/master/kategori-aplikasi/${editTarget.id}`, payload);
        showToast('ok', `"${form.nama}" diperbarui.`);
      } else {
        await api.post('/master/kategori-aplikasi', payload);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/master/kategori-aplikasi/${deleteTarget.id}`);
  ...
  return (
    <>
      ...
      <TableCard>
        <CrudHeader
          searchPlaceholder="Cari kategori aplikasi..."
          searchValue={search}
        ...
        <CrudTable<KategoriAplikasi>
          headers={['Nama Kategori', 'Kode Kategori', 'Aksi']}
          loading={loading}
