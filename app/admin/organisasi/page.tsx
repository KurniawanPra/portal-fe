...
import {
  inputCls, searchInputCls, labelCls, Toast, TableCard, ActiveToggle, PrimaryButton, FilterDropdown, CrudTable
...
export default function UnitOrganisasiPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 gap-1.5 sm:gap-x-6 sm:gap-y-2 sm:flex sm:items-center bg-white dark:bg-slate-900 px-3 sm:px-5 py-3 sm:py-4 rounded-xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm">
        {[
          ...
          return (
            <React.Fragment key={s.label}>
              <div className="flex items-center justify-center gap-1.5 sm:gap-2 min-w-0">
                <Icon className={`h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 ${s.color}`} />
                <span className="text-xs sm:text-sm font-bold text-slate-850 dark:text-white">{s.value}</span>
                <span className="text-[11px] sm:text-xs font-semibold text-slate-550 dark:text-slate-400 truncate">{s.label}</span>
              </div>
      ...
      <TableCard accentColor="via-amber-500/30">
        ...
        {/* Table */}
        <CrudTable<UnitOrganisasi>
          headers={['No', 'Kode', 'Nama Unit', 'Tipe', 'Parent Unit', 'Status', 'Aksi']}
          loading={loading}
          loadingText="Memuat data unit organisasi..."
          emptyText="Tidak ada unit organisasi yang sesuai."
          data={paginatedData}
          containerClassName="hide-scrollbar"
          renderRow={(u, idx) => (
            <tr key={u.id} className="group transition-colors duration-150 hover:bg-slate-50/50 dark:hover:bg-white/[0.02]">
              <td className="whitespace-nowrap px-5 py-3.5 font-mono text-xs font-bold text-slate-400 dark:text-slate-500">
                {(currentPage - 1) * itemsPerPage + idx + 1}
              </td>
              <td className="px-5 py-3.5">
                <span className="rounded-lg bg-slate-100 px-2.5 py-1 text-[11px] font-black uppercase text-amber-600 dark:bg-white/[0.06] dark:text-amber-400">{u.kode}</span>
              </td>
              ...
              <td className="px-5 py-3.5">
                <div className="flex flex-nowrap items-center justify-start gap-1">
                  <button onClick={() => openEdit(u)} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 transition-all hover:bg-amber-500/10 hover:text-amber-500 focus:outline-none dark:text-slate-500">
                    <Pencil className="h-3.5 w-3.5" />
                  </button>
                  <button onClick={() => setDeleteTarget(u)} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 transition-all hover:bg-rose-500/10 hover:text-rose-500 focus:outline-none dark:text-slate-500">
