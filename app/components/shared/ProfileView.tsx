...
export default function ProfileView({ isAdmin = false }: ProfileViewProps) {
  ...
  const [photoToast, setPhotoToast] = useState<AppToastState>(null);
  ...
  const handlePhotoUpload = async (file: File) => {
    ...
    try {
      ...
      await api.post('/auth/me/photo', fd);
      ...
      setPhotoToast({ type: 'ok', text: 'Foto profil berhasil diperbarui!' });
      setTimeout(() => setPhotoToast(null), 3000);
    } catch (err: any) {
      setPhotoToast({ type: 'err', text: err.message || 'Gagal mengunggah foto.' });
      setTimeout(() => setPhotoToast(null), 3000);
    } finally {
