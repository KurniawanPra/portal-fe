  ...
  return (
    <div className="space-y-6 max-w-5xl">
        ...
        <div className={`${isAdmin ? 'lg:col-span-3' : 'lg:col-span-2'} space-y-6`}>
          ...
          <div className="rounded-xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-6 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
            ...
            <form onSubmit={handlePasswordChange} className="space-y-4 max-w-md">
              ...
              <Button
                type="submit"
                className="bg-amber-500 hover:bg-amber-600 text-white dark:bg-amber-550 dark:hover:bg-amber-600 border-transparent shadow-sm shadow-amber-500/20"
              >
                Perbarui Password
              </Button>
            </form>
                      ...
                      <div key={pk.id} className="p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/20 dark:bg-slate-950/5 hover:bg-slate-50/50 dark:hover:bg-slate-950/15 transition-colors">
                        ...
                        {editingId !== pk.id && (
                          <div className="flex items-center gap-2 text-[10px] font-bold">
                            ...
                            <button
                              onClick={() => setPasskeyToDelete({ id: pk.id, name: pk.name })}
              ...
              <div className="border-t border-slate-100 dark:border-slate-800/80 pt-3.5">
                {registering ? (
                  <div className="space-y-3 bg-slate-50/50 dark:bg-slate-950/20 border border-slate-150 dark:border-slate-850 p-4 rounded-xl">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="bg-amber-500 hover:bg-amber-600 disabled:bg-slate-400 text-white rounded-lg px-3.5 py-1.5 text-xs font-semibold flex items-center gap-1.5 cursor-pointer disabled:cursor-not-allowed transition-colors shadow-sm shadow-amber-500/20"
                      >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-lg border border-transparent bg-amber-500 hover:bg-amber-600 px-3.5 py-2 text-xs font-semibold text-white transition-colors cursor-pointer shadow-sm shadow-amber-500/20"
                  >
                  ...
                  <div className="space-y-4">
                    {totpSetupMode ? (
                        ...
                        <div className="border-t border-slate-150 dark:border-slate-800/80 pt-3.5 flex flex-col sm:flex-row sm:items-center gap-3">
                          ...
                          <div className="flex items-end gap-2 sm:pt-4">
                            <button
                              ...
                              disabled={totpLoading}
                              className="bg-amber-500 hover:bg-amber-600 disabled:bg-slate-400 text-white rounded-lg px-3.5 py-2 text-xs font-semibold flex items-center gap-1.5 cursor-pointer disabled:cursor-not-allowed transition-colors shadow-sm shadow-amber-500/20"
                            >
                              {totpLoading && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
                              Aktifkan
                            </button>
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg border border-transparent bg-amber-500 hover:bg-amber-600 px-3.5 py-2 text-xs font-semibold text-white transition-colors cursor-pointer shadow-sm shadow-amber-500/20"
                      >
                        {totpLoading ? <Loader2 className="h-4 w-4 animate-spin shrink-0" /> : <KeyRound className="h-4 w-4 shrink-0" />}
      ...
      <DeleteConfirmModal
        ...
        onConfirm={() => passkeyToDelete && handleDeletePasskey(passkeyToDelete.id)}
        title="Hapus Passkey?"
        name={passkeyToDelete?.name}
      />

      <DeleteConfirmModal
        isOpen={totpDisableModalOpen}
        onClose={() => {
          setTotpDisableModalOpen(false);
          setTotpConfirmPassword('');
        }}
        ...
        deleting={totpLoading}
        description={
          <div className="space-y-3 mt-1">
            <p className="text-sm">Apakah Anda yakin ingin menonaktifkan Autentikasi Dua Faktor (2FA)? Akun Anda akan lebih rentan.</p>
            <div className="text-left mt-3">
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Masukkan Password Anda:</label>
              <input
                type="password"
                value={totpConfirmPassword}
                onChange={(e) => setTotpConfirmPassword(e.target.value)}
                className="w-full border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 rounded-lg px-3 py-2 outline-none text-xs font-semibold text-slate-800 dark:text-slate-200 focus:border-slate-400"
                placeholder="Password akun Anda"
              />
            </div>
          </div>
