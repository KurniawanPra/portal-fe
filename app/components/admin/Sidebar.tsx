        ...
        {menuGroups.map((group, gIdx) => {
          ...
          return (
            <SidebarGroup key={group.label} className={gIdx > 0 ? 'pt-2' : ''}>
              {state === 'collapsed' && gIdx > 0 && (
                <div className="px-2 mb-2">
                  <hr className="border-slate-200/60 dark:border-slate-800/40" />
                </div>
              )}
              ...
              {(isOpen || state === 'collapsed') && (
                <SidebarMenu className={cn(
                  ...
                  state === 'expanded' ? 'border-l border-slate-100 dark:border-slate-850 ml-3 pl-1.5' : 'pl-0'
                )}>
      ...
      <SidebarFooter>
        <div className={cn(
          ...
          <Link
            href="/admin/profile"
            className={cn(
              'flex items-center gap-2.5 overflow-hidden group hover:opacity-80 transition-opacity cursor-pointer',
            ...
            {state === 'expanded' && (
              <div className="grid flex-1 text-left text-sm leading-tight animate-fade-in overflow-hidden">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="shrink-0 rounded-full bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-800/40 px-1.5 py-px text-[8px] font-black uppercase tracking-wide text-rose-600 dark:text-rose-450">
                    Admin
                  </span>
                </div>
              </div>
