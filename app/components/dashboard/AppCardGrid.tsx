...
export function AppIcon({ name }: { name: string }) {
  ...
  const containerClass = "h-28 w-28 shrink-0 flex items-center justify-center transition-transform duration-300 bg-transparent";
        ...
        return (
          <div className={containerClass}>
            <img
              ...
              loading="lazy"
              decoding="async"
              className="h-24 w-24 shrink-0 rounded-2xl object-contain bg-transparent"
              onError={(e) => {
...
export default function AppCardGrid({ apps, searchQuery, showUuid = false, columns = 3, isEmployee = true }: AppCardGridProps) {
  ...
  const handleOpenApp = async (e: React.MouseEvent<HTMLButtonElement>, app: Aplikasi) => {
    ...
    const launchUrl = `/launch?appId=${encodeURIComponent(app.id)}&app=${encodeURIComponent(app.nama)}`;

    // Immediately notify live presence that user is opening this app
    api.post('/presence/heartbeat', {
      appId: app.id,
      appName: app.nama,
      currentPath: `/launch?appId=${app.id}`,
  ...
  return (
    <div className="w-full">
      {filteredApps.length === 0 ? (
        ...
        <StaggerContainer className={`grid grid-cols-1 gap-5 sm:gap-6 sm:grid-cols-2 ${columns === 4 ? 'md:grid-cols-3 lg:grid-cols-4' : 'md:grid-cols-2 lg:grid-cols-3'} items-stretch auto-rows-fr`} stagger={0.06}>
          {filteredApps.map((app) => {
            ...
            return (
              <AnimatedCard
                key={app.id}
                data-app-card
                className={`group perspective-1200 min-h-[13.5rem] h-full w-full rounded-[2rem] transition-all duration-[900ms] ease-[cubic-bezier(0.25,1,0.35,1)] ${isLaunching ? 'scale-[1.03] opacity-70 blur-[1px]' : ''}`}
              >
                {/* Inner 3D Container with Luxurious Slow 3D Flip */}
                <div className="relative w-full h-full rounded-[2rem] transition-transform duration-[900ms] ease-[cubic-bezier(0.25,1,0.35,1)] preserve-3d will-change-transform group-hover:[transform:rotateY(180deg)]">

                  {/* Front Face: App Store Style Icon & Application Name (Enhanced Neumorphic Aura) */}
                  <div className={`absolute inset-0 w-full h-full rounded-[2rem] border border-white/90 dark:border-white/[0.1] bg-gradient-to-br from-slate-50/95 via-white to-slate-100/90 dark:from-[#181e2d] dark:via-[#131824] dark:to-[#0e121c] backdrop-blur-2xl shadow-[8px_8px_20px_rgba(163,177,198,0.55),-8px_-8px_20px_rgba(255,255,255,1),inset_0_1.5px_1.5px_rgba(255,255,255,0.95)] dark:shadow-[8px_8px_24px_rgba(4,7,13,0.95),-6px_-6px_20px_rgba(36,48,72,0.45),inset_0_1.5px_1.5px_rgba(255,255,255,0.14)] flex flex-col items-center justify-center p-4 backface-hidden ${isLockedSso ? 'opacity-75' : ''}`}>
                    {isLockedSso && (
                      <div className="absolute top-3.5 right-3.5 bg-rose-500/10 dark:bg-rose-500/20 text-rose-600 dark:text-rose-450 p-1.5 rounded-full border border-rose-500/20 shadow-xs">
                        <Lock className="h-3.5 w-3.5" />
                      </div>
                    )}
                    <AppIcon name={app.icon} />
                    <span className="mt-1 text-sm sm:text-base font-black tracking-tight text-slate-800 dark:text-slate-100 text-center truncate w-full px-2 flex items-center justify-center gap-1">
                      {app.nama}
                    </span>
                    {isLockedSso && (
                      <span className="text-[9.5px] text-rose-500 dark:text-rose-450 font-bold mt-0.5 uppercase tracking-wider">
                        Khusus Karyawan
                      </span>
                    )}
                  </div>

                  {/* Back Face: Redesigned Layout (Enhanced Neumorphic Card) */}
                  <div className="relative w-full h-full min-h-[13.5rem] rounded-[2rem] border border-white/90 dark:border-white/[0.1] bg-gradient-to-br from-white via-slate-50/95 to-slate-100/90 dark:from-[#181e2d] dark:via-[#131824] dark:to-[#0e121c] backdrop-blur-2xl shadow-[8px_8px_20px_rgba(163,177,198,0.55),-8px_-8px_20px_rgba(255,255,255,1),inset_0_1.5px_1.5px_rgba(255,255,255,0.95)] dark:shadow-[8px_8px_24px_rgba(4,7,13,0.95),-6px_-6px_20px_rgba(36,48,72,0.45),inset_0_1.5px_1.5px_rgba(255,255,255,0.14)] p-4 sm:p-5 flex flex-col justify-between backface-hidden rotate-y-180 gap-2">
                    ...
                    {/* 2. Body (Description) */}
                    <p
                      className="text-[11px] text-slate-500 dark:text-slate-400 leading-snug mt-1 flex-1 font-medium line-clamp-3"
                      title={isLockedSso ? undefined : app.deskripsi}
                    >
                      {isLockedSso
